...
| ---------------- | ----------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- | ------- |
| `title`          | `string`{.type-string}                                                                    | The page title, displayed in listings and often rendered in templates.                      | —       |
| `published`      | `bool`{.type-bool}                                                                        | Determines if the page is visible to the public.                                            | `true`  |
...
| `routable`       | `bool`{.type-bool}                                                                        | Controls whether the page has a publicly accessible URL.                                    | `true`  |
| `listed`         | `bool`{.type-bool}                                                                        | If `false`, the page is to be considered hidden from page listings and/or navigation.       | `true`  |
| `searchable`     | `bool`{.type-bool}                                                                        | Controls if the page is to be included in search indexing.                                  | `true`  |
| `cacheable`      | `bool`{.type-bool}                                                                        | Whether the page can be cached.                                                             | `true`  |
| `orderable`      | `bool`{.type-bool}                                                                        | If the page can be manually reordered in the Panel.                                         | `false` |
| `allowChildren`  | `bool`{.type-bool}                                                                        | Controls whether the page can have subpages.                                                | `true`  |
...
| `headers`        | `array`{.type-keyword}                                                                    | Custom HTTP headers sent when serving the page (e.g., for caching or security).             | `[]`    |
| `responseStatus` | `int`{.type-number}                                                                       | Custom HTTP response status code (e.g., `404` for error pages).                             | `200`   |
| `metadata`       | `array`{.type-keyword}                                                                    | Associative array of custom meta tags, including Open Graph or SEO values.                  | `[]`    |
...
#### Cache-related fields {.is-new}

**Since 2.1.0**{.badge .badge-yellow}

Pages can define their cache settings using the following fields:

| Field        | Type                                                                                   | Description                                                                                                                                           | Default |
| ------------ | -------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| `cache.time` | <code><span class="type-number">int</span>\|<span class="type-null">null</span></code> | Duration (in seconds) for which the page should be cached. If not set, the global cache duration from [site options](../site-options/#cache) is used. | `null`  |
